...
?>
    ...
    SELECT 
        ...
        surat.tujuan,
        surat.driver,
        GROUP_CONCAT(stok_keluar_daftar.nama) AS nama, 
        GROUP_CONCAT(stok_keluar_daftar.kode_barang) AS kode_barang 
    FROM 
        surat
    INNER JOIN 
        stok_keluar_daftar 
    ON 
        surat.nota = stok_keluar_daftar.nota 
    WHERE 
        surat.nota LIKE '%$search%'
        OR surat.nosurat LIKE '%$search%'
        OR surat.tujuan LIKE '%$search%'
        OR surat.driver LIKE '%$search%' 
        OR stok_keluar_daftar.nama LIKE '%$search%' 
        OR stok_keluar_daftar.kode_barang LIKE '%$search%'  
    GROUP BY 
        surat.nota, surat.nosurat, surat.tanggal, surat.tujuan, surat.driver 
    ORDER BY 
        surat.nosurat 
    LIMIT $rpp";
