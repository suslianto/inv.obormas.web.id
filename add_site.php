...
function autoNumber() {
    global $conn;
    $query    = "SELECT MAX(CAST(SUBSTRING(kode, 2) AS UNSIGNED)) as max_id FROM pelanggan WHERE kode LIKE 'C%'";
    $result   = mysqli_query($conn, $query);
    $data     = mysqli_fetch_array($result);
    $id_max   = $data['max_id'];
    $sort_num = ($id_max === null) ? 1 : $id_max + 1;
    return sprintf("C%03d", $sort_num);
}
            ...
            <?php if ($chmod >= 2 || $_SESSION['jabatan'] == 'admin') : ?>
                ...
                if (!empty($no) && ($chmod >= 3 || $_SESSION['jabatan'] == 'admin')) {
                    ...
                    if ($fill = mysqli_fetch_assoc($hasil_edit)) {
                        ...
                        $nohp_form = $fill["notelp"]; // Menggunakan notelp sesuai database
                        $insert_mode = '3';
                    }
                }
                ?>
                                ...
                                <div class="box-body">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="text" class="form-control" id="nohp" name="nohp" value="<?php echo htmlspecialchars($nohp_form); ?>" placeholder="Masukan Nomor Telepon">
                                            </div>
                                        </div>
                                        <div class="form-group">
