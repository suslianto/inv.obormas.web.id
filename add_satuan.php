...
function autoNumber() {
    global $conn;
    $query    = "SELECT MAX(CAST(SUBSTRING(kode, 2) AS UNSIGNED)) as max_id FROM satuan WHERE kode LIKE 'S%'";
    $result   = mysqli_query($conn, $query);
    $data     = mysqli_fetch_array($result);
    $id_max   = $data['max_id'];
    $sort_num = ($id_max === null) ? 1 : $id_max + 1;
    return sprintf("S%03d", $sort_num);
}
    ...
    if ($insert == '3') { // Mode Update
        ...
        if ($chmod >= 2 || $_SESSION['jabatan'] == 'admin') {
            ...
            if (mysqli_num_rows($result_check) > 0) {
                $alert_script = "swal('Gagal!', 'Kode satuan sudah ada.', 'error');";
            } else {
                                ...
                                <div class="col-lg-6">
                                    <form class="form-horizontal" action="" method="post">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="col-sm-9">
                                                <input type="text" class="form-control" id="nama" name="nama" value="<?php echo htmlspecialchars($nama_form); ?>" placeholder="Masukan Satuan" maxlength="50" required>
                                            </div>
                                        </div>
                                        ...
                                        <div class="box-footer">
                                            ...
                                            <a href="<?php echo $halaman; ?>" class="btn btn-danger btn-flat" style="margin-left: 10px;"><i class="fa fa-close"></i> Batal</a>
                                        </div>
                                    </form>
                                </div>
                            </div>
